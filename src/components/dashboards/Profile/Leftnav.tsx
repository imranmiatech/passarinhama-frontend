                ...
                {NAV_SECTIONS.flatMap((s) =>
                    s.items.map((item) => (
                        <NavLink
                            ...
                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap flex-shrink-0 transition-colors border ${active === item.id
    ...
    <div className="space-y-6">
        {NAV_SECTIONS.map((section) => (
            <div key={section.section}>
                    ...
                    {section.items.map((item) => {
                        ...
                        return (
                            <NavLink
                                ...
                                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all group ${item.danger
                                        ? isActive
                            ...
                            >
                                <span
                                    className={`flex items-center justify-center w-8 h-8 rounded-xl flex-shrink-0 transition-colors ${item.danger
                                            ? isActive
                ...
                <p className="text-[#9BA3B2] font-bold">Zone sensible</p>
                <Link className="text-[#EF4444] font-normal flex gap-x-3 mt-5" to="/dashboard/profile/complete"> <Trash/>  <p>Supprimer le compte</p></Link>
            </div>
        ))}
    </div>
);
